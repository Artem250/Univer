...
public:
   ...
   void calcY(real** al, real* b, real* y, int n, int m);
...
//*
void matrix::calcY(real** al, real* b, real* y, int n, int m)
{
   for (int i = 0; i < n; i++)
   {
      double sum = 0.0;
...
//*/
/*
void matrix::calcY(real** al, real* di, real* b, real* y, int n, int m) {
   for (int i = 0; i < n; i++) {
      real s = 0;
      for (int j = i - m, j_band = 0, ij = i; j_band < m; j_band++, j++, ij++)
         if (j >= 0)
            s += al[i][j] * y[j];
      y[i] = (b[i] - s) / di[i];
   }
...
int main() {
   ...
   //matrix.toLent();
   matrix.createGilbert(matrix.al, matrix.di, 3, 2);
   matrix.createGilbert(matrix.au, matrix.di, 3, 2);
   ...
   for (int i = 0; i < matrix.n; i++)
      printf("%f\n", matrix.di[i]);
   //matrix.gauss();
   //matrix.calcY(matrix.al, matrix.b, matrix.y, matrix.n, matrix.m);
