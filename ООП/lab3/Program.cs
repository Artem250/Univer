...
static LnFun Ln(Expr operand) => new(operand);
...
//var expr1 = (x - 4) * (3 * x + y * y) / 5 - (-1);
//var expr1 = x + x1;
var expr1 = Pow(x, 5.01);
...
public abstract class Expr : IExpr
{
    ...
    public static Negative operator -(Expr operand) => new(operand);
...
public class Constant : Expr
{
    ...
    public override string ToString() => Value < 0 ? $"({Value})" : $"{Value}";
...
public class Variable : Expr
{
    ...
    public override IEnumerable<string> Variables => _variables;
    public override bool IsConstant => false;
    public override bool IsPolynomial => true;
    public override int PolynomialDegree => 1;
    public override string ToString() => Name;
    public override Expr Derivative(Variable x) => x.Name == this.Name ? 1 : 0;
...
public class Negative : UnaryOperation
{
    ...
    public override Expr Derivative(Variable x) => -Operand.Derivative(x);
...
public class PowFun : Function
{
    ...
    public override bool IsConstant => Left.IsConstant && Right.IsConstant;
    public override bool IsPolynomial => Left.IsPolynomial && Right is Constant rightConst && rightConst.Value == Math.Floor(rightConst.Value);
    //public override int PolynomialDegree =>
    //Left.IsConstant ? 0 : (IsPolynomial ? Convert.ToInt32(((Constant)Right).Value) : -1); // правая часть больше 0
    ...
    {
        return new PowFun(Left, Right) * new LnFun(Left) * Right.Derivative(x) +
...
public class LogFun : Function
{
    ...
    public override Expr Derivative(Variable x) => (Left * new LnFun(Left) * Right.Derivative(x) -
        Left.Derivative(x) * Right * new LnFun(Right)) /
        (Left * new PowFun(new LnFun(Left), 2) * Right);
}


public class LnFun : UnaryOperation
{
    public LnFun(Expr operand) : base(operand) { }
    public override bool IsConstant => Operand.IsConstant;
    ...
    public override double Compute(IReadOnlyDictionary<string, double> variableValues)
    {
        ...
        if (val <= 0) throw new Exception("The natural logarithm makes sense when b > 0");

        return Math.Log(Operand.Compute(variableValues));
    }
    public override string ToString() => $"Ln({Operand})";


    public override Expr Derivative(Variable x) => 1 / Operand * Operand.Derivative(x);
}
